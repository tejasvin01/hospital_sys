  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const patientCount = usersResponse.data.filter(user => 
          user.role?.toLowerCase() === 'patient'
        ).length;
        
        const doctorCount = usersResponse.data.filter(user => 
          user.role?.toLowerCase() === 'doctor'
        ).length;

        const receptionistCount = usersResponse.data.filter(user => 
        ...
        const todayAppointments = appointmentsResponse.data.filter(appointment => 
        ...
        setRecentUsers([
          { id: 1, name: 'Sarah Williams', email: 'sarah.w@example.com', role: 'patient' },
          { id: 2, name: 'James Anderson', email: 'james.a@example.com', role: 'patient' },
          { id: 3, name: 'Dr. Elizabeth Parker', email: 'elizabeth.p@example.com', role: 'doctor' },
          { id: 4, name: 'Dr. Robert Chen', email: 'robert.c@example.com', role: 'doctor' },
        ]);
      }
    };
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="relative">
              <button
                ...
                <img src="https://public.readdy.ai/ai/img_res/4769923e2a4b10f063d40bf3f71c0205.jpg"
                  alt="Admin"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 bg-white shadow-sm overflow-y-auto relative">
          <nav className="p-4 pb-20">
            <div className="space-y-2">
              {[
                { icon: 'fas fa-chart-line', text: 'Dashboard', id: 'dashboard' },
                { icon: 'fas fa-user-md', text: 'Users', id: 'users' },
                { icon: 'fas fa-calendar-alt', text: 'Appointments', id: 'appointments' },
                { icon: 'fas fa-file-medical', text: 'Reports', id: 'reports' },
                { icon: 'fas fa-money-bill-wave', text: 'Billing', id: 'billing' },
                { icon: 'fas fa-file-invoice', text: 'Invoices', id: 'invoices' }
              ].map(item => (
                <button
                  key={item.id}
                  onClick={() => handleMenuClick(item.id)}
                  className={`flex items-center space-x-3 w-full p-3 rounded-lg cursor-pointer whitespace-nowrap
                  ${selectedMenu === item.id ? 'bg-indigo-50 text-indigo-600' : 'text-gray-600 hover:bg-gray-50'}`}>
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          <div className="grid grid-cols-2 md:grid-cols-2 gap-6 mb-6">
            {[
              { title: 'Total Patients', value: stats.totalPatients, icon: 'fas fa-users', color: 'bg-blue-500' },
              { title: 'Appointments Today', value: stats.appointmentsToday, icon: 'fas fa-calendar-check', color: 'bg-green-500' },
              { title: 'Active Doctors', value: stats.activeDoctors, icon: 'fas fa-user-md', color: 'bg-purple-500' },
              {title: 'Total Receptionists', value: stats.totalReceptionists, icon: 'fas fa-id-card', color: 'bg-amber-500' }
            ].map((stat, index) => (
              <div key={index} className="bg-white rounded-lg shadow-sm p-8">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-base text-gray-500 font-medium">{stat.title}</p>
